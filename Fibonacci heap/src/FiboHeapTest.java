...
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int NumOfHeap2Trees = heap2.getNumOfTrees();
        heap.meld(heap2);
        if (heap.size() != size + size2) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.getNumOfTrees() != numOfTrees + NumOfHeap2Trees) {
            System.out.println("heap1 Trees =" +numOfTrees);
            System.out.println("heap2 Trees =" +NumOfHeap2Trees);
            System.out.println("sum after meld =" +heap.getNumOfTrees());
